...
jsondata.replace('<I>', '')
jsondata.replace('</I>', '')
jsondata.replace(unnecessary_punctuation, ' ')
...
def optimal_Z(optimal_F, optimal_R, optimal_C):
    optimal_F = 0 
    optimal_R = 0 
    optimal_C = 0
    
    for optimal_F,optimal_R,optimal_C in range (1,100): 
        while (1000*optimal_F + 600*optimal_R + 750*optimal_C) <= 18000: #this is gold constraint
            optimal_F += 1
            optimal_R += 1 
            optimal_C += 1
            
        while (500*optimal_F + 750*optimal_R + 300*optimal_C) <= 12000: #this is lumber constraint
            optimal_F += 1
            optimal_R += 1 
            optimal_C += 1

        while (50*optimal_F + 45*optimal_R + 75*optimal_C) <= 1500: #this is food constraint
            optimal_F += 1
            optimal_R += 1 
            optimal_C += 1
...
#find the number
def optimal_F(optimal_Z):
    if 1000*optimal_F <= 18000 and 500*optimal_F <= 12000 and 50*optimal_F <= 1500:
        return optimal_F(344)

def optimal_R(optimal_Z):
    if 600*optimal_R <= 18000 and 750*optimal_R <= 12000 and 45*optimal_R <= 1500:
        return optimal_R(344)

def optimal_C(optimal_Z):
    if 750*optimal_C <= 18000 and 300*optimal_C <= 12000 and 75*optimal_C <= 1500:
        return optimal_C(344)
...
def Collatz(start_number):
    ...
    while start_number != 1: 
        if start_number % 2 == 0: #this number is even
...
collatz_length = len(str(collatz_sequence))
   max_collatz = max(collatz_sequence)
   sequence_sum = sum(collatz_sequence)
